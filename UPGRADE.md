...
Edit `composer.json`, then update the three packages together. All three are on Packagist now,
so the `vcs` entries that named them can leave `repositories` (`composer config --unset
repositories.<name>`). A `path` entry for a checkout beside your project is fine for development,
but the lock file must be resolved from Packagist, never from a `path` dist.

| Package | Before | After |
|---|---|---|
| `idct/adminata` | `dev-main` | `dev-main`, from Packagist (the rename is on `main`; adminata is untagged until its first release) |
| `idct/adminata-doctrine-orm-admin-bundle` | `^1.0` | `^2.0`, from Packagist |
