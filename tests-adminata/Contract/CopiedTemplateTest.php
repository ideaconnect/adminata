...
final class CopiedTemplateTest extends ContractTestCase
{
    ...
    {
        $templates = [
            self::ORM_VIEWS.'/Form/form_admin_fields.html.twig',
            self::ORM_VIEWS.'/Form/filter_admin_fields.html.twig',
        ];
        ...
        foreach ([
            ...
            $templates[] = \sprintf('src/Resources/views/Block/%s.html.twig', $block);
        }
    ...
    public function testTheAssociationIncludesAreDeclaredDeferred(): void
    {
        $theme = (string) file_get_contents(
            self::root().'/'.self::ORM_VIEWS.'/Form/form_admin_fields.html.twig'
        );
        ...
        $deferred = array_flip(array_filter(
            array_map(trim(...), explode("\n", (string) file_get_contents(self::root().'/tests-adminata/Contract/deferred-templates.txt'))),
            static fn (string $line): bool => '' !== $line && !str_starts_with($line, '#')
        ));

        foreach ($matches[1] as $include) {
            static::assertArrayHasKey(
                'src/Resources/views/'.$include,
                $deferred,
