...
final class TemplatePathTest extends ContractTestCase
{
    ...
    {
        foreach (self::NAMESPACES as $namespace => $views) {
            static::assertDirectoryExists(
                \sprintf('%s/%s', self::root(), $views),
    ...
    public function testTheNumberOfTemplatesIsWhatThePlanCounts(): void
    {
        ...
        foreach (array_unique(self::NAMESPACES) as $views) {
            $directory = \sprintf('%s/%s', self::root(), $views);
        ...
        // `ModelListType` widget P4-03 lifted out of the form theme. Three of those 150 are the
        // ORM storage layer's, which is a package of its own since PLAN/01 P17 and is counted here
        // through its installed copy.
        static::assertSame(150, $found, 'The admin bundle and the ORM storage layer ship 150 templates.');
    }

    /**
     * Every PHP file and Twig template this bundle ships.
     *
     * @return iterable<string>
     */
    private static function shippedSources(): iterable
    {
        $sources = glob(self::root().'/src', \GLOB_ONLYDIR);
    ...
    private static function referencedTemplates(): array
    {
        ...
        foreach (self::directories(self::root().'/src') as $directory) {
            $iterator = new \RecursiveIteratorIterator(new \RecursiveDirectoryIterator($directory, \FilesystemIterator::SKIP_DOTS));
    ...
    private static function resolve(string $template): string
    {
        ...
        return \sprintf('%s/%s/%s', self::root(), self::NAMESPACES[$matches[1]], $matches[2]);
    }
}
