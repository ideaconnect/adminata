...
| 2026-09-12 | **"drop any sonata references going towards naming Adminata under IDCT namespace (`IDCT\Adminata\`), yet keep in history, readme etc. references that it was once sonata and that we are grateful for the original project."** Every name this plan froze under its Sonata spelling — namespace, bundle class, config roots, service ids, routes, Twig namespace and blocks, form types, translation domain, markup hooks, Stimulus identifiers, cookies, asset path — is adminata's own, `replace` becomes `conflict`, the storage layers follow (ORM 2.0, MongoDB 7.0), and the upstream sync translates through an engine that also serves applications. The contracts of 02 are re-issued under the new names, not weakened | [PLAN/v2/](v2/README.md), the whole of it: decisions N1–N22, the rule map, attribution, consumers, tasks R0–R6, appendix A; `UPGRADE.md` at the root |
