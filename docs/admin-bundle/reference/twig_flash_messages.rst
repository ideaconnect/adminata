...
The classes are ``IDCT\Adminata\FlashMessage\`` and the layout already renders them on every
...
.. note::
    ...
    ``info``, which become ``adm-alert-success`` and friends. Dismissal is the ``adminata-dismiss``
    Stimulus controller — there is no ``data-dismiss`` data API, because there is no jQuery to
    read it. adminata's own ``@Adminata/FlashMessage/render.html.twig`` maps Sonata's
...
.. code-block:: yaml
    ...
    # config/packages/adminata_twig.yaml
...
``IDCT\Adminata\FlashMessage\FlashManagerInterface`` (the service id is
``adminata.twig.flashmessage.manager``)::
...
.. code-block:: html+twig
    ...
    {# check each types #}
    {% for type in adminata_flashmessages_types() %}

        {# get messages from current type #}
        {% set messages = adminata_flashmessages_get(type) %}

        {# display flash message; adminata_flashmessages_class returns the configured CSS class #}
        {% for message in messages %}
            <div class="adm-alert adm-alert-{{ adminata_flashmessages_class(type, 'info') }}"
                 role="alert"
                 {{ stimulus_controller('adminata-dismiss') }}>
                <div class="adm-alert__body">{{ message|raw }}</div>
                <button type="button"
                        class="adm-alert__dismiss"
                        aria-label="{{ 'message_close'|trans({}, 'AdminataBundle') }}"
                        {{ stimulus_action('adminata-dismiss', 'dismiss', 'click') }}>
