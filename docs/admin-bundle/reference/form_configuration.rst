...
(:doc:`/admin-bundle/getting_started/installation`). Their classes are ``Sonata\AdminBundle\`` —
``Form\Type\``, ``Form\DataTransformer\``, ``Form\EventListener\``, ``Validator\`` and
``Test\`` — and ``SonataAdminBundle`` registers the form services, the datepicker form theme and
the ``sonata_form`` configuration root.

What the form layer has of its own:

* ``sonata_form`` is a configuration root of its own, in its own
  ``config/packages/sonata_form.yaml``; the tree is below.
* Service ids are ``sonata.form.*`` — ``sonata.form.type.array``, ``type.boolean``,
...
* The form templates are in the admin bundle's view directory, under ``@SonataAdmin/Form/``, and
  that is the path every default names — ``datepicker.html.twig`` is added to ``twig.form_themes``
  as ``@SonataAdmin/Form/datepicker.html.twig``. So a form template is overridden exactly like any
  other admin template: a file of the same name under ``templates/bundles/SonataAdminBundle/``,
  with ``@!SonataAdmin/Form/…`` reaching the shipped one. ``@SonataForm`` is kept as a
  compatibility alias of the same directory, for templates outside adminata that still say
  ``@SonataForm/…``: ``@SonataForm/Form/datepicker.html.twig`` and
  ``@SonataAdmin/Form/datepicker.html.twig`` are one file.
...
— are in ``SonataAdminBundle`` with the admin bundle's own, and that is the default of every
...
.. note::
    ...
    Coming from Sonata? The ``Sonata\Form\`` classes are ``Sonata\AdminBundle\`` here — the map is
    ...
    ``translations/SonataAdminBundle.<locale>.xliff``, ids unchanged. Service ids and
    ``config/packages/sonata_form.yaml`` need no edit, and a template of yours that says
    ``@SonataForm/…`` still resolves. A ``templates/bundles/SonataFormBundle/`` directory is read
    by nothing any more: its files move to ``templates/bundles/SonataAdminBundle/``.
...
and that theme reads ``sonata_admin.options.form_type``, which is where an admin application sets
the style:
...
``sonata_form`` keeps a ``form_type`` node of its own for forms built outside an admin class.
``SonataFormExtension`` prepends ``sonata_admin.options.form_type`` onto it, so the two agree
without being written twice; set it directly only if you have no ``sonata_admin`` configuration:
...
The one node that is **not** prepended is :doc:`twig_configuration`'s ``sonata_twig.form_type``,
...
``bin/console config:dump-reference sonata_form`` prints the whole tree with its defaults. It is
one node:
