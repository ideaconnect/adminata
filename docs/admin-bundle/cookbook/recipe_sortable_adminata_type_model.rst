...
But the ``ModelType`` (or adminata_type_model) extends from choice, so this
...
So we are going to start by creating this ``UserBundle\Admin\UserHasExpectationsAdmin.php``::
    ...
    final class UserHasExpectationsAdmin extends AbstractAdmin
    {
        ...
        {
            $form
                ->add('expectation', 'adminata_type_model', ['required' => false])
                ->add('position', 'hidden')
            ;
        }
...
.. code-block:: xml
    ...
    <service id="user.admin.user_has_expectations" class="UserBundle\Admin\UserHasExpectationsAdmin">
        <tag name="adminata.admin" model_class="UserBundle\Entity\UserHasExpectations" manager_type="orm" group="UserHasExpectations" label="UserHasExpectations"/>
    </service>

Now update the ``UserBundle\Admin\UserAdmin.php`` by adding the ``adminata_type_model`` field::

    protected function configureFormFields(FormMapper $form): void
    {
        $form
            ->add('userHasExpectations', 'adminata_type_model', [
