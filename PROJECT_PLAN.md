...
- [x] **R6-02 follow-up · Packagist** — the owner published `idct/adminata` (`dev-main` only, until the first tag), `idct/adminata-admin-mongodb-bundle` and `idct/adminata-doctrine-orm-admin-bundle` on 2026-09-12; every `vcs` entry is gone from this manifest, the storage layers' and the panel's, and the documents say the packages come from Packagist. Still the owner's: `idct/sonata-admin-mongodb-bundle` is not yet marked abandoned — Packagist keeps updating it from the renamed repository, so it serves v7.0.0 under the old name too.
