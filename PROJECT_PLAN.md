...
- [x] **P6-04 · The package directories: drop the upstream repo scaffolding** · S · depends: P6-01
  - Each of the seven forks arrived as a whole repository and kept its root: a `Makefile`,
    a `bin/console`, a `README.md` with Packagist badges for the package adminata replaces,
    a `CONTRIBUTING.md` pointing at Sonata's process, upstream `UPGRADE-*.md` for major versions
    that predate the fork, and `.editorconfig`, `.yamllint`, `.gitattributes`, `.gitignore`,
    `.php-cs-fixer.dist.php`, `.readthedocs.yaml`, `.symfony.bundle.yaml` — 83 files, every one
    auto-generated by `sonata-project/dev-kit`, none referenced by anything here, and all
    superseded by a root that already governs `packages/**`. A package is a directory of this
    repository, not a repository, and it now looks like one: `src/`, `tests/`, `LICENSE`,
    `CHANGELOG.md`.
  - Kept deliberately: the upstream `LICENSE` (MIT attribution, and the only one of the four that
    ships in the Composer archive) and the upstream `CHANGELOG.md`, which `CHANGELOG-sonata.md`
    links as the inherited history.
  - The two rules that were doing real work moved rather than vanished: the per-package
    `.gitignore` kept the packages' own test-app artefacts (`tests/App/public/bundles`,
    `tests/App/config/reference.php`) out of the index, and the per-package `.gitattributes` kept
    their dotfiles and markdown out of the dist archive. Both are now single globbed rules in the
    root files, and the root's dead per-package rules (`phpstan*`, `phpunit.xml.dist`, `rector.php`
    — matching nothing since M0) are gone with them.
  - Every `upstream/exclude/<name>.txt` gained the scaffolding paths, so a later
    `make upstream-sync` cannot quietly restore them one file at a time; `UPSTREAM.md` says what a
    package directory holds and why.
  - Accept: `git archive` lists **1040 files before and after** — an application's `vendor/` is
...
- [x] **R6-02 follow-up · Packagist** — the owner published `idct/adminata` (`dev-main` only, until the first tag) and `idct/adminata-admin-mongodb-bundle` on 2026-09-12; the storage layers and the panel dropped their `vcs` entries for the two, and the documents say where each package comes from. Still the owner's: `idct/adminata-doctrine-orm-admin-bundle` is not on Packagist (consumers keep its `vcs` entry), and `idct/sonata-admin-mongodb-bundle` is not yet marked abandoned — Packagist keeps updating it from the renamed repository, so it serves v7.0.0 under the old name too.
