 ...
 */
final class AdminataExporterExtension extends Extension
{
    /**
     * @param mixed[] $config
     */
    ...
    public function load(array $configs, ContainerBuilder $container): void
    {
        ...
        $loader->load('exporter_services.php');
    ...
    private function configureExporter(ContainerBuilder $container, array $config): void
    {
        foreach (['csv', 'json', 'xls', 'xlsx', 'xml'] as $format) {
            if (\in_array($format, $config['default_writers'], true)) {
                $container->getDefinition('adminata.exporter.writer.'.$format)->addTag(
                    'adminata.exporter.writer'
                );
            }
        }
    }
