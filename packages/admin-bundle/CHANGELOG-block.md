...
> **adminata note.** This is `sonata-project/block-bundle`'s own changelog, up to the imported tag
> 5.4.0. The bundle was merged into this package directory on 2026-09-06 — its classes are
> `Sonata\AdminBundle\` now and there is no `SonataBlockBundle` — so its changelog sits beside the
