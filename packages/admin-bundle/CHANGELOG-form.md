...
> **adminata note.** This is `sonata-project/form-extensions`'s own changelog, up to the imported
> tag 2.7.0. The bundle was merged into this package directory on 2026-09-06 — its classes are
> `Sonata\AdminBundle\` now and there is no `SonataFormBundle` — so its changelog sits beside the
> admin bundle's own rather than replacing it. adminata's changes are in the repository's
