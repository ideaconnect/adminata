...
> **adminata note.** This is `sonata-project/exporter`'s own changelog, up to the imported tag
> 3.4.0. The library was merged into this package directory on 2026-09-07 — its classes are
> `Sonata\AdminBundle\Exporter\` now and there is no `SonataExporterBundle` — so its changelog sits
> beside the admin bundle's own rather than replacing it. adminata's changes are in the repository's
> [CHANGELOG.md](../../CHANGELOG.md); the index of upstream histories is
